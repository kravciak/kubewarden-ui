...
test('Install UI extension', async({ page, ui }) => {
  ...
  await test.step('Enable extension support', async() => {
    if (RancherUI.isVersion('<2.9')) {
      await extensions.enable({ rancher: inst.ui_from === 'prime', partners: false })
    }
    // Wait for default list of extensions
    if (inst.ui_from === 'prime') {
      if (RancherUI.isVersion('>=2.9')) {
...
test('Install Kubewarden', async({ page, ui, nav }) => {
  test.skip(inst.kw_mode === 'fleet')

  const kwPage = new KubewardenPage(page)
  await kwPage.installKubewarden({ version: inst.kw_mode === 'upgrade' ? inst.upMap[0].controller : undefined })
...
test('Upgrade Kubewarden', async({ page, nav }) => {
  test.skip(inst.kw_mode !== 'upgrade')
  test.slow()
  ...
  for (const chart of ['controller', 'crds', 'defaults']) {
    await apps.checkChart(`rancher-kubewarden-${chart}`, inst.upMap[0][chart])
  }

  // Keep track of last upgraded version
  let last: AppVersion = inst.upMap[inst.upMap.length - 1]

  await test.step('Upgrade predefined versions', async() => {
    for (let i = 0; i < inst.upMap.length - 1; i++) {
      await nav.kubewarden()
      await kwPage.upgrade({ from: inst.upMap[i], to: inst.upMap[i + 1] })
    }
  })
