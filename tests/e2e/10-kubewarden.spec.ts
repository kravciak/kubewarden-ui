...
test('Install Kubewarden', { tag: '@in' }, async({ page, ui, nav }) => {
...
test('Install Kubewarden by Fleet', { tag: '@in' }, async({ page }) => {
  test.skip(conf.kw_mode !== 'fleet')
  test.slow()
...
test('Add Policy Catalog Repository', { tag: '@in' }, async({ page, ui, nav }) => {
