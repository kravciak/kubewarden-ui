...
export class Shell {
  ...
  async expectScreenshots(names: string[], options?: { clip?: { x: number, y: number, width: number, height: number } }) {
    ...
    //   try {
    //     return await expect(this.page).toHaveScreenshot(names[i], options)
    //   } catch {}
    // }
    // await expect(this.page).toHaveScreenshot(names[names.length - 1], options)
    ...
    //   throw new Error(`None of the expected screenshots matched: ${names.join(', ')}`, { cause: errors })
    // }
  }
